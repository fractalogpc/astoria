namespace Console.Commands
{
  ...
  public class _ToggleBuildMenuCommand : ConsoleCommand
  {
    ...
    public override string Command => "togglebuildmenu";
    ...
    public override bool Process(string[] args) {
      ...
      if (TryValue(value)) {
        if (value == 1) {
          ...
          BackgroundInfo._toggleBuildMenu = true;
        }
        else {
          _currentSuccessMessage = BuildSuccessMessage("disabled");
          BackgroundInfo._toggleBuildMenu = false;
        }
    ...
    private string BuildCallbackMessage() =>
      $"build menu toggle is {(BackgroundInfo._enableCheats ? 1 : 0)}";
    
    private string BuildSuccessMessage(string arg) =>
      $"{arg} toggle build menu.";
