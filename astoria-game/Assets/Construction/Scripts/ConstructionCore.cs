...
public class ConstructionCoreNew : InputHandlerBase
{
  #region Variables
  ...
  [SerializeField] private LayerMask _ConstructionObjectWallLayer;
  ...
  protected override void InitializeActionMap()
  {
    ...
    RegisterAction(_inputActions.Player.Rotate, _ => ToggleRotating(), () => ToggleRotating());
...
//         if (NearOtherWall(hit.point, out ConstructionObjectWall wallScript))
//         {
...
//   private bool NearOtherWall(Vector3 position, out ConstructionObjectWall wallScript) {
//     Collider[] colliders = Physics.OverlapSphere(position, _wallSnapDistance, _ConstructionObjectWallLayer);
//     wallScript = null;
...
//       ConstructionObjectWall tempWall = c.GetComponent<ConstructionObjectWall>();
...
//   private bool SnapToWall(Vector3 position, ConstructionObjectWall wallScript) {
...
//     int tempJointIndex;
//     if (_isRotating) {
//       closestTempWallJoint = _tempObject.GetComponent<ConstructionObjectWall>().GetJointWorldSpace(stashedTempJointIndex);
//       tempJointIndex = stashedTempJointIndex;
//     } else {
//       closestTempWallJoint = _tempObject.GetComponent<ConstructionObjectWall>().GetClosestJointWorldSpace(closestOtherWallJoint, out tempJointIndex);
...
//     Vector3 tempWallDirection = _tempObject.GetComponent<ConstructionObjectWall>().GetJointWorldSpace((tempJointIndex == 0) ? 0 : 1) - _tempObject.GetComponent<ConstructionObjectWall>().GetJointWorldSpace((tempJointIndex == 0) ? 1 : 0);
...
//     LayerMask effectiveLayer = layer ?? (_constructionLayer | _ConstructionObjectWallLayer | LayerMask.GetMask("Default")); // Not great practice but works
